...
<script>
    ...
    function cancel(s, dateTime, type) {
        const selected = (type === 1 ? confirm("예약이 취소됩니다.") : confirm("주문이 취소됩니다."));
        let params = 'rest=' + s + '&date=' + dateTime + '&type=' + type;
        if (type === 0)
            params += ('&did=' + $('input[name=dishID]').val());
        if (selected) {
                        ...
                        <table id="orders">
                            ...
                            <tr>
                                ...
                                <td><input type="button" value="주문 취소" onclick="cancel('<%=rs.getString(2)%>',
                                        '<%=DataBaseUtil.replaceSpace(rs.getString(5))%>', 0)">
                                </td>
                            </tr>
                            <% } %>
                            </tbody>
                                ...
                                <td>
                                    <input type="button" id="cancellation" value="예약 취소"
                                           onclick="cancel('<%=r.getCode()%>',
                                                   '<%=DataBaseUtil.replaceSpace(r.getReservedTime())%>', 1)">
                                </td>
                            </tr>
                            <% } %>
                            </tbody>
